...
struct ContentView: View {
    ...
    private var assistanceView: some View {
        ...
        return Group {
            if scheme.showBBB {
                ...
                let defaultWeight = max(assistWeightBinding(for: selectedLift).wrappedValue, a.defaultBarWeight)
                ...
                ForEach(1...3, id: \.self) { setNum in
                    AssistSetRow(
                        ...
                        hasBarWeight: a.defaultBarWeight > 0,
                        ...
                        perSide: a.defaultBarWeight > 0 ?
                            (a.useEZBar ? calculator.plates(target: workoutState.getAssistWeight(lift: selectedLift.rawValue, week: currentWeek, set: setNum) ?? defaultWeight, barWeight: 25)
                             : calculator.plates(target: workoutState.getAssistWeight(lift: selectedLift.rawValue, week: currentWeek, set: setNum) ?? defaultWeight))
                        : [],
                        roundTo: roundTo,
                        onCheck: { checked in
    ...
    private func currentWorkoutMetrics() -> (est: Double, totalVol: Int, mainVol: Int, bbbVol: Int, assistVol: Int) {
        ...
        let defaultAssistW = max(assistWeightBinding(for: selectedLift).wrappedValue, assistDef.defaultBarWeight)
        var assistVol = 0.0
        for setNum in 1...3 {
    ...
    private struct AssistSetRow: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 6) {
                HStack(alignment: .firstTextBaseline) {
                    ...
                    VStack(alignment: .trailing, spacing: 2) {
                        if hasBarWeight {
                            ...
                            Text("Bodyweight / DBs").font(.caption).foregroundStyle(.secondary)
                        }
                    }
                }
                .onChange(of: done) { _, new in
                ...
                HStack(spacing: 16) {
                    // Weight adjustment (only if using barbell)
                    if hasBarWeight {
                        HStack(spacing: 4) {
                            Text("Weight:").font(.caption).foregroundStyle(.secondary)
                            Button {
